        ...
        if (levelType === '1') {
            ...
            if (type === 0) {
                ...
                if (Math.random() > 0.5) {
                    ...
                    let denom = `${a*a}${v}^2 - ${b*b}`;
                    let correctStr = `\\frac{${numCorrect}}{${denom}}`;
                    
                    // 隱藏屬性全為 false，系統會自動將每一行串接等號 "="，形成 Step-by-step 教學
                    let steps = [
                        { text: `${term1} ${opStr} ${term2}`, hide: false },
                        { text: `\\frac{${A}(${a}${v} - ${b})}{(${a}${v} + ${b})(${a}${v} - ${b})} ${opStr} \\frac{${B}(${a}${v} + ${b})}{(${a}${v} + ${b})(${a}${v} - ${b})}`, hide: false },
                        { text: `\\frac{${A}(${a}${v} - ${b}) ${opStr} ${B}(${a}${v} + ${b})}{(${a}${v} + ${b})(${a}${v} - ${b})}`, hide: false },
                        { text: `\\frac{${A*a}${v} - ${A*b} ${fmtC(op*B*a)}${v} ${fmtC(op*B*b)}}{${denom}}`, hide: false },
            ...
            let steps = [
                ...
                { text: `\\frac{${A}${v} - ${A*a} ${fmtC(op*B)}${v} ${fmtC(op*B*a)}}{(${v} - ${a})^2(${v} + ${a})}`, hide: false },
                { text: correctStr, hide: false }
            ];
            let eqCorrectHtml = wrapHint(msgCorrect, buildEq(steps));
            ...
            let options = [
                { text: `\\( \\displaystyle ${correctStr} \\)`, isCorrect: true, hint: eqCorrectHtml },
                { text: `\\( \\displaystyle \\frac{${numW1}}{${denomCorrect}} \\)`, isCorrect: false, hint: wrapHint(msgFracSign + "<div class='text-sm text-slate-500 mb-2'>(提示：通分母時小心補乘的項)</div>", buildEq(steps)) },
                { text: `\\( \\displaystyle \\frac{${numW2}}{${denomCorrect}} \\)`, isCorrect: false, hint: wrapHint(msgFracSign + "<div class='text-sm text-slate-500 mb-2'>(提示：分子展開計算錯誤)</div>", buildEq(steps)) },
